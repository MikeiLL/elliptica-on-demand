...
class AdminEnqueueCest {
	...
	function enqueue_admin_scripts(AcceptanceTester $I) {
		...
		$I->seeInPageSource('mz-mindbody-classes-settings-script');
		$I->seeInPageSource('mz-mindbody-classes-admin-script');
	}

	function enqueue_admin_styles(AcceptanceTester $I) {
		$I->wantTo('access to the plugin settings page and check the style enqueue');
		$I->amOnPage('/wp-admin/admin.php?page=mz-mindbody-classes');
		$I->seeInPageSource('mz-mindbody-classes-settings-styles-css');
		$I->seeInPageSource('mz-mindbody-classes-admin-styles-css');
	}
